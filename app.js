...
function renderToday() {
  ...
  remEl.textContent = money(ts.remaining);
  ...
  document.getElementById("week-allotted").textContent = money(ws.allotted);
  document.getElementById("week-spent").textContent = money(ws.spent);
  document.getElementById("week-saved").textContent = money(ws.saved);
  document.getElementById("week-saved").className = "wc-value " + (ws.saved < 0 ? "rust" : "sage");
  const pct = ws.allotted > 0 ? Math.min(100, (ws.spent / ws.allotted) * 100) : 0;
  const fill = document.getElementById("week-progress");
  fill.style.width = pct + "%";
  fill.style.background = pct > 100 ? "var(--rust)" : "var(--gold)";
